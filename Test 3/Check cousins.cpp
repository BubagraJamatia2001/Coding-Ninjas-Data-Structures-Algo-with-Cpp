...
int depth(BinaryTreeNode<int>* root, int data, int dep){ 
    if(root == NULL)
        return 0; 
    if(root->data == data)
        return dep; 
    int ld = depth(root->left, data, dep+1); 
    if (ld != 0)
        return ld; 
    return depth(root->right, data, dep+1); 
} 

bool isCousin(BinaryTreeNode<int>* root, int p, int q){
    if(root == NULL)
        return NULL;
    int lp = depth(root,p,1);
    int lq = depth(root,q,1);
    if(lp==lq && !(isSibling(root,p,q))) 
        return true; 
    else 
        return false;
}
